...
import { ImplementationError, SyntaxError } from "../core/error";
import { Range, Token, Source as RasperSource } from "../core/core";
import { nameFrom } from "../core/util";
...
const Source = class ABNFSource extends RasperSource {
...
const Parser = class ABNFParser {
  ...
  static fromString( str, entry ) {
    return this.fromSrc( new Source( str ), entry );
  }
  
  static fromSrc( src, entry = src.tokens[0].val ) {
    let self = new Parser( entry );
    
    self.src = src;
    self.tokens = {};
    
    if ( src.tokens[1].val !== "=" ) {
      let token = src.tokens[1];
      throw token.syntaxError( "Expected token '=', found '" + token.val + "'" );
    }
    ...
    let ruleTokens;
    for ( let i = 0, len = src.tokens.length - 1; i < len; i++ ) {
      let token = src.tokens[ i ];
      let next = src.tokens[ i + 1 ];
      
      if ( next.val === "=" ) {
        
        ruleTokens = [];
        self.tokens[ nameFrom( token ) ] = ruleTokens;
        
      } else if ( next.val === "=/" ) {
        
        ruleTokens = self.tokens[ nameFrom( token ) ];
        ruleTokens.push( new Token( "/", src, next.line, next.char ) );
    ...
    let lastToken = src.tokens[ src.tokens.length - 1 ];
